"""
Módulo 6: Breach Monitor
Daemon que corre en background y monitorea emails contra HIBP.
Envía notificaciones al OS y/o email cuando detecta un breach nuevo.
Persiste estado en ~/.datasec/monitor_state.json
"""
...
HIBP_HEADERS = {
    ...
    "hibp-api-key": ""  # Set via: datasec monitor add-email --api-key YOUR_KEY
}
...
def _breach_fingerprint(breaches: list) -> str:
    """Hash of breach names+dates — changes only when new breaches appear."""
    key = "|".join(sorted(f"{b['Name']}:{b.get('BreachDate','')}" for b in breaches))
...
def _fetch_breaches(email: str, api_key: str) -> list | None:
    """Returns breach list, empty list if clean, None on error."""
...
def _notify_os(title: str, message: str):
    ...
    try:
        ...
        pass  # plyer not available or no display — silent fail


def _notify_email(smtp_cfg: dict, to_email: str, new_breaches: list):
    """Send email alert for new breaches."""
    if not smtp_cfg.get("host") or not smtp_cfg.get("user"):
        return

    breach_lines = "\n".join(
        f"  - {b['Name']} ({b.get('BreachDate','?')[:7]}): {', '.join(b.get('DataClasses',[])[:3])}"
...
Recommended actions:
  1. Change your password on affected sites immediately
...
"""
    ...
    msg["To"]      = to_email
    msg["Subject"] = f"[datasec] New breach detected: {to_email}"
...
def _check_all_emails(state: dict) -> dict:
    ...
    for email, meta in emails.items():
        ...
        if current_fp != previous_fp and previous_fp != "":
            ...
            current_names  = {b["Name"] for b in breaches}
            new_names      = current_names - previous_names
            new_breaches   = [b for b in breaches if b["Name"] in new_names]

            console.print(f"\n[bold red]⚠ NEW BREACH for {email}:[/bold red]")
            for b in new_breaches:
                console.print(f"  [red]•[/red] {b['Name']} — {', '.join(b.get('DataClasses',[])[:4])}")

            _notify_os(
                f"datasec: New breach for {email}",
                f"{len(new_breaches)} new breach(es): {', '.join(b['Name'] for b in new_breaches[:3])}"
            )
...
def monitor_remove_email(email: str):
    ...
    if email not in state["emails"]:
        console.print(f"[yellow]{email} not found in monitoring list.[/yellow]")
        return
...
def monitor_configure(api_key: str = None, interval: int = None,
                      smtp_host: str = None, smtp_user: str = None,
                      smtp_pass: str = None, smtp_port: int = 465):
    state = _load_state()
    if api_key:
    ...
    if smtp_host:
        state["smtp"] = {
            ...
            "password": smtp_pass, "port": smtp_port
        }
...
def monitor_status():
    ...
    for email, meta in emails.items():
        ...
        count_str = f"[red]{count}[/red]" if isinstance(count, int) and count > 0 else f"[green]{count}[/green]"
        table.add_row(email, count_str, last)
...
def monitor_run(once: bool = False):
    """Start the monitor. once=True runs a single check and exits."""
    state = _load_state()

    console.print(Panel(
        "[bold green]datasec breach monitor running[/bold green]\n"
        f"[dim]Monitoring {len(state.get('emails', {}))} email(s) "
        f"every {state.get('interval_hours', 24)}h[/dim]\n"
